...
int main(){
	...
	while(getline(cin,line)){
		...
		if(entry.sent_mask=="ffff" || entry.recv_mask=="ffff"){
			continue;
		}
	...
	ofstream pdr;
	pdr.open ("pdr.txt", ios::out | ios::app);
	cout<<endl;
	for(int i=4; i<8; i++){
		double result = (double)count[i]/(finalSeq[i]-startSeq[i]+1);
		pdr<<setw(10)<<result;
		cout<<"node "<<i+2<<": "<< result <<endl;
