...
def output_error(line):
   ...
   for line_index in range(len(line)/num_bytes_per_line+1):
      ...
      #FILE = open('NF.txt','a')
      FILE = open('30may','a')
        #v1=(binascii.hexlify(pkt['src_addr']),'\n')
      FILE.writelines(wawa)
...
def print_ipv6(pkt):
   ...
   FILE = open('30may','a')
   #v1=(binascii.hexlify(pkt['src_addr']),'\n')
   v=(binascii.hexlify(pkt['payload']),'\n')
   #v=v1+v2
   line=str(v)
#   FILE.writelines(binascii.hexlify(pkt['payload']))
   FILE.writelines(v)
...
#socketThreadHandler           = socketThread(8020)
